...
//   __device__ SweepMarker(int aabbId, ccd::gpu::Aabb &a) {
//     id = aabbId;
//     x = a.min.x;
//     length = a.max.x - x;
//   }
...
__global__ void build_index(ccd::gpu::Aabb *boxes, int N, int *index);
__global__ void print_sort_axis(ccd::gpu::Aabb *axis, int C);
__global__ void retrieve_collision_pairs(const ccd::gpu::Aabb *const boxes,
...
// for balancing
__global__ void build_checker(ccd::gpu::Scalar3 *sortedmin, int2 *outpairs,
                              int N, int *count, int guess);
__global__ void create_ds(ccd::gpu::Aabb *boxes, ccd::gpu::Scalar2 *sortedmin,
...
__global__ void retrieve_collision_pairs2(const MiniBox *const mini, int *count,
                                          ...
                                          int guess);
__global__ void calc_variance(ccd::gpu::Aabb *boxes, ccd::gpu::Scalar3 *var,
                              int N, ccd::gpu::Scalar3 *mean);
__global__ void calc_mean(ccd::gpu::Aabb *boxes, ccd::gpu::Scalar3 *mean,
                          int N);
...
// for pairing
__global__ void create_ds(ccd::gpu::Aabb *boxes, ccd::gpu::Scalar3 *sortedmin,
                          MiniBox *mini, int N, ccd::gpu::Scalar3 *mean);
...
__global__ void create_rankbox(ccd::gpu::Aabb *boxes, RankBox *rankboxes,
                               int N);
...
// __global__ void sweepqueue(int2 * queue, const ccd::gpu::Aabb * boxes, int *
// count, int guess, int N, int TotBoxes, int start, unsigned * end, int2 *
// overlaps);
__global__ void sweepqueue(int2 *queue, const ccd::gpu::Aabb *boxes, int *count,
