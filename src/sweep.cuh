...
__global__ void retrieve_collision_pairs(const ccdgpu::Aabb* const boxes, int * count, int2 * overlaps, int N, int guess, int nbox, int start = 0, int end = INT_MAX);
...
__global__ void create_ds(ccdgpu::Aabb * boxes, float3 * sortedmin, MiniBox * mini, int N, Dimension axis);
__global__ void retrieve_collision_pairs2(const MiniBox* const mini, int * count, int2 * inpairs, int2 * overlaps, int N, int guess);
__global__ void calc_variance(ccdgpu::Aabb * boxes, float3 * var, int N, float3 * mean);
__global__ void calc_mean(ccdgpu::Aabb * boxes, float3 * mean, int N);
...
// for pairing
__global__ void create_ds(ccdgpu::Aabb * boxes, float3 * sortedmin, MiniBox * mini, int N, float3 * mean);
...
__global__ void sweepqueue(int2 * queue, ccdgpu::Aabb * boxes, int * count, int guess, int * d_N, int N, int TotBoxes, int start, unsigned * end, int2 * overlaps);
