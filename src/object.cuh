...
__device__ struct Cell {
  ...
  __device__ Cell(float left, float right, float upper, float lower,
                  Aabb *d_boxes, int N, int gid) // left, right, upper, lower
  {
    ...
    //     &&
    //          parent->boxes[i].min.y > lower && parent->boxes[i].max.y <
    //          upper)
    //     {
    ...
    for (int i = 0; i < Nboxes; i++) {
      ...
      if (boxes[i].min.x < min.x && boxes[i].max.x > max.x) {
        ...
        for (int j = 0; j < Nboxes; j++) {
          // if (boxes[j].max.x >= boxes[i].min.x && boxes[j].max.x >=
          // boxes[i].min.x )
  ...
  __device__ void Cut(Cell *nextcells) {
    ...
    past = boxes[Nboxes / 2];
    // Cell cellLU(this, min.x, past.min.x, max.y, (past.max.y-past.min.y)/2);
    // //Cell, Left, Right, Upper, Lower Cell cellLL(this, min.x, past.min.x,
    // (past.max.y-past.min.y)/2, min.y); Cell cellRU(this, past.max.x, max.x,
    // max.y, (past.max.y-past.min.y)/2); Cell cellRL(this, past.max.x, max.x,
    // (past.max.y-past.min.y)/2, min.y);

    // Cell(float left, float right, float upper, float lower, Aabb * d_boxes,
    // int N )
    ...
    Cell cellL(min.x, (past.max.x - past.min.x) / 2, max.y, min.y, lboxes, N,
               2 * id + 1);
    Cell cellR((past.max.x - past.min.x) / 2, max.x, max.y, min.y, rboxes, N,
               2 * id + 2);

    // Cell cellOL(this, min.x, past.min.x, max.y, min.y);
    // Cell cellOR(this, past.max.x, max.x max.y, min.y);

    // Cell cellIU(this, past.min.x, past.max.x, max.y,
    // (past.max.y-past.min.y)/2); Cell cellIL(this, past.min.x, past.max.x,
    // (past.max.y-past.min.y)/2, min.y); enqueue.push(cellLU);
    // enqueue.push(cellLL);
    // enqueue.push(cellRU);
    // enqueue.push(cellRL);
    nextcells[0] = cellL;
