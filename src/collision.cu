...
{
        __shared__ Aabb s_objects[4*2*BLOCK_SIZE_1D];
    
        int threadRowId = 4*blockIdx.x * blockDim.x + threadIdx.x;
        int threadColId = 4*blockIdx.y * blockDim.y + threadIdx.y;
        ...
        for (int i=0; i < 8; i++)
        {
            for (int j=i+1; j < 8; j++)
            {
                bool collides = does_collide(compBoxes[i],compBoxes[j]);
                check_add_overlap(collides,compBoxes[i],compBoxes[j], count, overlaps, G);
            }
        }
