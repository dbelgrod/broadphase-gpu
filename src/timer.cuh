...
void recordLaunch(char *tag, int gs, int bs, size_t mem,
  ...
  if (error != cudaSuccess) {
    ...
    if (err != cudaSuccess)
      spdlog::trace("Device-kernel launch failure {:s}",
...
template <typename... Arguments>
void recordLaunch(char *tag, int gs, int bs, void (*f)(Arguments...),
                  Arguments... args) {
  size_t mem = 0;
...
__device__ Fun recordLaunch(char *tag, Fun (*f)(Arguments...),
                            Arguments... args) {
