...
__global__ void square_sum(int *d_in, int *d_out, int *d_count, int N,
                           int start, int end) {
...
void merge_local(const tbb::enumerable_thread_specific<vector<int>> &storages,
...
void run_sweep_multigpu(int N, int devcount) {
  ...
  tbb::parallel_for(0, devices_count, 1, [&](int &device_id) {
    ...
    square_sum<<<grid, block>>>(d_in_solo, d_out, d_count, N, range_start,
                                range_end);
    ...
    gpuErrchk(
      cudaMemcpy(out, d_out, sizeof(int) * count, cudaMemcpyDeviceToHost));
