...
__global__ void run(ll *in, ll *out, int N) {
  ...
  //     int rand1, rand2;
  //     /* CUDA's random number library uses curandState_t to keep track of the
  //     seed value
  //      we will store a random state for every thread  */
  //     curandState_t state;

  //   /* we have to initialize the state */
  //     curand_init(0, /* the seed controls the sequence of random values that
  //     are produced */
  //                 0, /* the sequence number is only important with multiple
  //                 cores */ tid, /* the offset is how much extra we advance in
  //                 the sequence for each call, can be 0 */ &state);
